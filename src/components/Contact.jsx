...
export default function Contact() {
      ...
      for (let i = 0; i < maxParticles; i++) {
        for (let j = i + 1; j < maxParticles; j++) {
          ...
          if (dist < maxDistance) {
            ...
            ctx.strokeStyle = `rgba(99,102,241,${0.5 * (1 - dist / maxDistance)})`;
            ctx.lineWidth = 1;
            ctx.stroke();
          }
        }
  ...
  const handleMouseEnter = (e) => {
    ...
    if (
      ...
      dir = fromLeft ? "left" : "right";
    } else {
      dir = fromTop ? "top" : "bottom";
    }
    setDirection(dir);
  };
  ...
  return (
    <section
      id="contact"
      ref={sectionRef}
      className="relative flex items-center justify-center bg-gradient-to-b from-gray-900 to-black px-6 sm:px-8 py-12 sm:py-20 min-h-screen overflow-hidden"
    >
      {/* Canvas background */}
      <canvas
        ref={canvasRef}
      ...
      {/* Centered container */}
      <div className="max-w-5xl w-full mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 relative z-10">
        {/* Left info panel */}
        <div className="text-white flex flex-col justify-center space-y-5 px-2 sm:px-4">
          <span
            ref={headerRef}
            className="text-indigo-400 font-semibold tracking-widest text-xs sm:text-sm select-none"
          >
            Contact Me
          </span>
          <h2
          ...
          <p className="text-white/80 text-base sm:text-lg leading-relaxed">
            Let’s create something great together! Drop me a message, and I’ll
        ...
        <form
          ref={formRef}
          className="space-y-5 sm:space-y-6 px-2 sm:px-4 flex flex-col justify-center"
        ...
        >
          ...
          <button
            ...
            className="group relative w-full py-3 sm:py-4 rounded-lg border border-indigo-500 overflow-hidden text-white font-semibold text-base sm:text-lg transition-transform active:scale-95 shadow-md"
          >
            <span
              className={`absolute inset-0 bg-indigo-500 transform transition-transform duration-300 ease-out ${getAnimationClasses(
                direction
              )}`}
            ></span>
